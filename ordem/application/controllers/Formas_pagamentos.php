...
//        exit();
    ...
    public function add() {
        ...
        if ($this->form_validation->run()) {
            ...
            $data = array(
                'titulo' => 'Cadastrar forma de pagamentos',                
            );
            ...
            if ($this->form_validation->run()) {
                ...
                if ($this->db->table_exists('vendas')) {
                    ...
                    if ($forma_pagamento_ativa == 0 && $this->core_model->get_by_id('vendas', array('venda_forma_pagamento_id' => $forma_pagamento_id, 'venda_status' => 0))) {
                        $this->session->set_flashdata('info', 'Essa forma de pagamento não pode ser desativada, pois está sendo utilizada em <i class="fas fa-tools"></i>&nbsp; Vendas');
                        redirect('modulo');
                    }
                }
                ...
                if ($this->db->table_exists('ordem_servicos')) {
                    ...
                    if ($forma_pagamento_ativa == 0 && $this->core_model->get_by_id('ordem_servicos', array('ordem_servico_pagamento_id' => $forma_pagamento_id, 'ordem_servico_status' => 0))) {
                        $this->session->set_flashdata('info', 'Essa forma de pagamento não pode ser desativada, pois está sendo utilizada em <i class="fas fa-tools"></i>&nbsp; Ordem de servico');
                        redirect('modulo');
                    }
                }
                ...
                $data = array(
                    'titulo' => 'Editar forma de pagamentos',
