...
class Ordem_servicos extends CI_Controller {
    ...
    public function edit($ordem_servico_id = NULL) {
        ...
        if (!$ordem_servico_id || !$this->core_model->get_by_id('ordens_servicos', array('ordem_servico_id' => $ordem_servico_id))) { //ordens errado -> ordem_
    ...
    public function del ($ordem_servico_id = NULL){
        ...
        if ($this->core_model->get_by_id('ordens_servicos', array('ordem_servico_id' => $ordem_servico_id, 'ordem_servico_status' => 0))) { 
            $this->session->set_flashdata('error', 'Não e possível excluir uma ordem de serviço em aberto');
            redirect('os');
        }
    ...
    public function pdf($ordem_servico_id = NULL) {
        ...
        if (!$ordem_servico_id || !$this->core_model->get_by_id('ordens_servicos', array('ordem_servico_id' => $ordem_servico_id))) { //ordens errado -> ordem_
            ...
            $file_name = 'O.&nbsp;' . $ordem_servico->ordem_servico_id;
            ...
            $html .= '<p align="right" style="font-size: 15px">O.S Nº&nbsp;' . $ordem_servico->ordem_servico_id . '</p>';

            $html .= '<p>'
                    . '<strong>Contratante: </strong>' . $ordem_servico->cliente_nome_completo . '<br/>'
            ...
            $html .= '<td style="border-top: solid #dd 1px"><strong>Valor final</strong></td>';
            $html .= '<td style="border-top: solid #dd 1px">' . $valor_final_os->os_valor_total . '</td>';
